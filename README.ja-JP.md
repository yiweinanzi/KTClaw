...
サイドバー検索は検索ボタンまたは `Ctrl/Cmd+K` から開けるようになり、会話、エージェント、ページ、さらにキャッシュ済みのチャット履歴本文まで検索できます。会話とエージェントの右クリックメニューからは対応する会話を Markdown として直接エクスポートできます。Activity ページも、生ログ表示からフィルタ可能な構造化イベントカード + raw line 展開へ更新されました。
