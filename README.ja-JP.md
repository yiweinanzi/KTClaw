...
<p align="center">
  <a href="#機能">機能</a> •
  <a href="#なぜclawxなのか">なぜKTClawなのか</a> •
  <a href="#はじめに">はじめに</a> •
...
**KTClaw**は、強力なAIエージェントと日常のユーザーとの間のギャップを埋めます。[OpenClaw](https://github.com/OpenClaw)をベースに構築されており、コマンドラインによるAIオーケストレーションを、アクセスしやすく美しいデスクトップ体験に変換します。ターミナルは不要です。

ワークフローの自動化、AI搭載チャネルの管理、インテリジェントなタスクのスケジューリングなど、KTClawはAIエージェントを効果的に活用するために必要なインターフェースを提供します。

KTClawはベストプラクティスのモデルプロバイダーが事前設定されており、Windowsおよび多言語設定をネイティブにサポートしています。もちろん、**設定 → 詳細設定 → 開発者モード**から高度な設定を微調整することもできます。
...
AIエージェントの構築にコマンドラインの習得は不要であるべきです。KTClawはシンプルな哲学のもとに設計されました：**強力な技術には、あなたの時間を尊重するインターフェースがふさわしい。**
...
KTClawは公式の**OpenClaw**コアを直接ベースに構築されています。別途インストールを必要とせず、アプリケーション内にランタイムを組み込むことで、シームレスな「バッテリー同梱」体験を提供します。
...
`@agent` で別のエージェントを選ぶと、KTClaw はデフォルトエージェントを経由せず、そのエージェント自身の会話コンテキストへ直接切り替えます。各エージェントのワークスペースは既定で分離されていますが、より強い実行時分離は OpenClaw の sandbox 設定に依存します。
チャット入力欄では `/new`、`/stop`、`/agent`、`/cwd`、`/memory`、`/cron`、`/settings`、`/export` などの slash commands も使え、キーボード中心でローカル操作を素早く実行できます。
サイドバー検索は検索ボタンまたは `Ctrl/Cmd+K` から開けるようになり、会話、エージェント、ページ、さらにキャッシュ済みのチャット履歴本文まで検索できます。会話はサイドバーでピン留めでき、会話とエージェントの右クリックメニュー、さらに `/export` から対応する会話を Markdown として直接エクスポートできます。Activity ページも、生ログ表示からフィルタ可能な構造化イベントカード + raw line 展開 + live 自動更新へ更新されました。

### 🧪 リッチ Markdown レンダリング
チャットの Markdown は通常の数式に加えて KaTeX `mhchem` の化学式にも対応し、コードハイライト、ローカルファイルリンク、コードコピーも維持されています。
...
KTClaw はドキュメント処理スキル（`pdf`、`xlsx`、`docx`、`pptx`）もフル内容で同梱し、起動時に管理スキルディレクトリ（既定 `~/.openclaw/skills`）へ自動配備し、初回インストール時に既定で有効化します。追加の同梱スキル（`find-skills`、`self-improving-agent`、`tavily-search`、`brave-web-search`、`bocha-skill`）も既定で有効化されますが、必要な API キーが未設定の場合は OpenClaw が実行時に設定エラーを表示します。  
...
### 🌙 アダプティブテーマ
ライトモード、ダークモード、またはシステム同期テーマ。KTClawはあなたの好みに自動的に適応します。

### 🚀 自動起動設定
**設定 → 通用** から **システム起動時に自動起動** を有効化すると、ログイン後に KTClaw が自動的に起動します。
...
[Releases](https://github.com/ValueCell-ai/KTClaw/releases)ページから、お使いのプラットフォーム向けの最新リリースをダウンロードしてください。
...
KTClawを初めて起動すると、**セットアップウィザード**が以下の手順をガイドします：
...
KTClawには、Electron、OpenClaw Gateway、またはTelegramなどのチャネルがローカルプロキシクライアントを介してインターネットにアクセスする必要がある環境向けに、組み込みのプロキシ設定が含まれています。
...
- 高度なプロキシフィールドが空の場合、KTClawは`プロキシサーバー`にフォールバックします。
- プロキシ設定を保存すると、Electronのネットワーク設定が即座に再適用され、ゲートウェイが自動的に再起動されます。
- KTClawはTelegramが有効な場合、プロキシをOpenClawのTelegramチャネル設定にも同期します。
...
KTClawは、**デュアルプロセス + Host API 統一アクセス**構成を採用しています。Renderer は単一クライアント抽象を呼び出し、プロトコル選択とライフサイクルは Main が管理します：
...
- **Node.js**: 22以上（LTS推奨）
- **パッケージマネージャー**: Corepack 経由で固定版 `pnpm@10.31.0` を使用 (`corepack enable && corepack prepare pnpm@10.31.0 --activate`)
...
# コード品質
pnpm run lint             # ESLint のチェックのみを実行
pnpm run lint:fix         # ESLint を自動修正付きで実行
pnpm typecheck            # TypeScriptの型チェック

# テスト
pnpm test                 # ユニットテストを実行
pnpm run test:e2e         # Playwright E2E テストを実行
pnpm run test:e2e:headed  # Playwright E2E テストを headed モードで実行
...
コミュニティからのコントリビューションを歓迎します！バグ修正、新機能、ドキュメントの改善、翻訳など、あらゆる貢献がKTClawをより良くするのに役立ちます。
...
KTClawは優れたオープンソースプロジェクトの上に構築されています：
...
KTClaw パートナープログラムを開始します。特に、カスタム AI エージェントや自動化ニーズを持つより多くの顧客に KTClaw を紹介してくださるパートナーを募集しています。

パートナーの皆さまには、見込みユーザーや案件との接点づくりを担っていただき、KTClaw チームは技術サポート、カスタマイズ、統合を全面的に提供します。
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=ValueCell-ai/KTClaw&type=Date" alt="スター履歴チャート" />
</p>
...
KTClawは[MITライセンス](LICENSE)の下でリリースされています。本ソフトウェアの使用、変更、配布は自由に行えます。
