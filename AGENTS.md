...
KTClaw is a cross-platform **Electron desktop app** (React 19 + Vite + TypeScript) providing a GUI for the OpenClaw AI agent runtime. It uses pnpm as its package manager (pinned version in `package.json`'s `packageManager` field).
...
| Lint (ESLint, check-only) | `pnpm run lint` |
| Lint (ESLint, auto-fix) | `pnpm run lint:fix` |
...
- **`pnpm run lint` / `pnpm run lint:fix` race condition**: If `pnpm run uv:download` was recently run, ESLint may fail with `ENOENT: no such file or directory, scandir '/workspace/temp_uv_extract'` because the temp directory was created and removed during download. Simply re-run lint after the download script finishes.
...
- **Runtime session persistence**: `electron/services/session-runtime-manager.ts` persists legacy `transcript`, structured `history`, and derived `executionRecords` together. Older saved runtime records may not have `executionRecords`; hydrate them from persisted `history` instead of treating the field as required.
