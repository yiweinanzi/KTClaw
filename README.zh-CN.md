...
侧边栏搜索支持通过搜索入口或 `Ctrl/Cmd+K` 打开，并且已经可以搜索会话、分身、页面以及缓存的聊天历史正文。会话支持在侧边栏置顶，会话和分身的右键菜单以及聊天页顶部现在也支持直接导出对应对话 Markdown。Activity 页面也已从原始日志文本升级为可过滤、可展开 raw line、支持 live 自动刷新的结构化事件视图。
